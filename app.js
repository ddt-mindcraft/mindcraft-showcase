...
{kicker:'02 / 작업 워크플로우',title:'자료에서 결과까지,<br>확인하며 한 흐름으로.',description:'자료 확인 → 문서 작성 → 저장 승인 → 기록. 요청과 결과를 확인할 수 있는 작업 흐름을 연결합니다.',detail:'이번 요청에 사용할 자료를 직접 확인하고, 작성된 결과를 읽은 뒤 파일 저장을 승인합니다. 선택한 자료·결과·실행 기록이 연결되어 어떤 작업을 했는지 다시 확인할 수 있습니다. 아래 웹 체험은 설계·리뷰·테스트가 결과에 주는 의미를 설명하는 별도 예시입니다. <a href="./details.html#story">조건을 바꾸며 계산·검사 체험하기 ↗</a>'},
{kicker:'03 / 지식 시스템',title:'어제의 작업을,<br>오늘의 출발점으로.',description:'지난 작업의 결과와 출처를 남깁니다. 다시 쓸 자료를 골라 다음 작업에 활용합니다.',detail:'작업 재개는 멈춘 작업을 이어가는 기능, 지식 재사용은 이전 결과를 새 작업에 활용하는 기능입니다. 결과의 내용과 출처를 확인해 사용 가능으로 선택한 자료만 후속 요청에 활용합니다. 내가 선택한 자료를 기준으로 다음 작업의 맥락을 구성합니다.'},
