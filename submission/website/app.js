...
{kicker:'02 / 작업 워크플로우',title:'만드는 데서 끝내지 않고,<br>요청대로 됐는지 확인.',description:'자료 확인 → 문서 작성 → 저장 승인 → 기록. 요청과 결과를 확인할 수 있는 작업 흐름을 연결합니다.',detail:'설계 → 리뷰 → 실행 → 테스트를 사용자 작업에 적용하는 것이 핵심 방향입니다. r3는 문서 작업의 자료·출처 확인과 승인·기록을 제공합니다. 독립 설계 리뷰와 실행 테스트를 자동 연결하는 전체 루프는 다음 단계이며, 아래는 그 원리를 설명하는 예시입니다. <a href="./details.html#story">조건을 바꾸며 계산·검사 체험하기 ↗</a>'},
...
{kicker:'04 / 모델 역할 분담',title:'깊이 생각할 때와,<br>가볍게 처리할 때를 나눠서.',description:'일반 작업과 복잡한 계획에 모델을 나눠 지정합니다. 필요한 곳에 고성능·저비용 모델을 선택해 비용과 성능의 균형을 추구합니다.',detail:'사용자가 지정한 모델을 Planner(계획·복잡한 분석)와 Worker(일반 작업)에 배정하는 방식입니다. 모델 하나를 두 용도로 함께 쓸 수도 있습니다. 작업 용도에 따라 사용할 역할을 선택합니다. 단계별 자동 전환 전체를 뜻하지 않으며, 사용 중 임의 모델로 교체하지 않습니다. 절감률·응답 속도는 아직 측정하지 않았습니다.'},
