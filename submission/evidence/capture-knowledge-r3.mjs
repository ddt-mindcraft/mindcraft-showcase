...
const base=path.resolve('demo-evidence/knowledge-r3'),root=path.join(base,'session-'+Date.now()),folder=path.join(root,'온담 작업실'),profile=path.join(root,'profile'),temp=path.join(root,'temp'),shots=path.join(base,'screenshots');
...
try{
 ...
 await page.getByTestId('workspace-capture-button').click();await page.getByTestId('workspace-dialog-input').fill('온담 북클럽 준비 기준');await page.getByTestId('workspace-dialog-submit').click();
 await page.getByTestId('nav-knowledge').click();await page.getByTestId('knowledge-card').filter({hasText:'온담 북클럽 준비 기준'}).click();await expect(page.getByTestId('knowledge-promote-button')).toBeVisible();await shot('06-knowledge-review');
 await page.getByTestId('knowledge-promote-button').click();await page.getByRole('button',{name:'사용 가능한 자료',exact:true}).click();await expect(page.getByTestId('knowledge-card').filter({hasText:'온담 북클럽 준비 기준'})).toBeVisible();
 await page.getByTestId('nav-home').click();await page.getByTestId('home-request-input').fill('온담 북클럽 준비 기준을 참고해서 다음 모임 준비안을 정리해줘.');await page.getByTestId('home-start-button').click();
 const reused=page.locator('.source-card').filter({hasText:'온담 북클럽 준비 기준'});await expect(reused).toBeVisible({timeout:30000});await expect(reused.locator('input')).toBeChecked();await shot('07-knowledge-reuse');
 await page.getByTestId('workspace-context-confirm-button').click();await expect(page.locator('.status.completed')).toBeVisible({timeout:30000});
 if(errors.length||requests.length)throw Error(JSON.stringify({errors,requests}));
 await fs.writeFile(path.join(base,'capture-report.json'),JSON.stringify({status:'PASS',executableSha256:sha,sourceCommit:'26eb06f43d3e02aa1b0db04b85e1e870f239a417',build:'build-z9OSa3',mode:'offline',mockedIPC:false,modelCalls:0,networkRequests:requests,rendererErrors:errors,viewport:{width:1440,height:1000},steps:['first setup','3 source cards','completed result','approved file saved and contents verified','history reopened','result captured as knowledge candidate','knowledge promoted by user','new request includes selected knowledge','new request completed'],captures},null,2));
